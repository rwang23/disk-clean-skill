...
> Windows 回收站或其他平台的 Trash；会先呈现报告，再询问用户是否进入单独的 destructive 阶段。
...
不足的对象保持不动。永久删除不能替代备份或恢复测试。清空回收站/Trash 永远是报告
之后、由用户确认的单独 destructive 阶段。
